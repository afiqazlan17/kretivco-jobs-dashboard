-- Auto-generated Staff ID (EMP-001, EMP-002, ...) for every user row.
...
BEGIN
  IF NEW.staff_id IS NULL THEN
    NEW.staff_id := 'EMP-' || LPAD(nextval('staff_id_seq')::text, 3, '0');
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  FOR r IN SELECT id FROM users ORDER BY created_at ASC LOOP
    UPDATE users SET staff_id = 'EMP-' || LPAD(nextval('staff_id_seq')::text, 3, '0') WHERE id = r.id;
  END LOOP;
END;
$$;
