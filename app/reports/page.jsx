...
export default function Reports(){
  ...
  const funnel={pot:filtered.filter(j=>j.status==="potential").length,new:filtered.filter(j=>j.status==="new").length,asg:filtered.filter(j=>j.status==="assigned").length,act:filtered.filter(j=>j.status==="active").length,comp:comp.length};
  const fTotal=funnel.pot+funnel.new+funnel.asg+funnel.act+funnel.comp;
      ...
      <div className="page">
        ...
        <div className="content">
          ...
          <div className="g2">
            ...
            <div className="card"><div className="card-title">Conversion Funnel</div><div className="section-label" style={{marginBottom:16}}>Potential → Completed</div>
              <div className="funnel-row">
                {[{l:"Potential",n:funnel.pot,v:formatRMShort(filtered.filter(j=>j.status==="potential").reduce((s,j)=>s+(j.estimation_value||0),0)),c:"#6366F1"},{l:"New",n:funnel.new,v:formatRMShort(filtered.filter(j=>j.status==="new").reduce((s,j)=>s+(j.estimation_value||0),0)),c:"#F59E0B"},{l:"Assigned",n:funnel.asg,v:formatRMShort(filtered.filter(j=>j.status==="assigned").reduce((s,j)=>s+(j.estimation_value||0),0)),c:"#3A86FF"},{l:"Active",n:funnel.act,v:formatRMShort(filtered.filter(j=>j.status==="active").reduce((s,j)=>s+(j.estimation_value||0),0)),c:"#10B981"},{l:"Selesai",n:funnel.comp,v:formatRM(totalFinal),c:"#6B7280",last:true}].map((s,i)=>(
          ...
          <div className="card"><div className="card-title" style={{marginBottom:16}}>Pecahan Bulanan</div>
            <div style={{overflowX:"auto"}}><table><thead><tr><th style={{textAlign:"left"}}>Bulan</th><th style={{textAlign:"center"}}>Jobs</th><th style={{textAlign:"right"}}>Anggaran</th><th style={{textAlign:"right"}}>Final</th>{deptKeys.map(k=><th key={k} style={{textAlign:"center",color:DEPT[k].color,fontWeight:600}}>{DEPT[k].code}</th>)}</tr></thead><tbody>
              ...
              <tr className="tr-total"><td>Jumlah</td><td style={{textAlign:"center"}}>{totalJobs}</td><td style={{textAlign:"right"}}>{formatRM(totalEst)}</td><td style={{textAlign:"right",color:"#10B981"}}>{formatRM(totalFinal)}</td>{deptKeys.map(d=><td key={d} style={{textAlign:"center",color:DEPT[d].color}}>{deptBr[d]?.count||0}</td>)}</tr>
            </tbody></table></div>
          </div>
