    ...
    <Modal w={500} onClose={guardedClose}>
      ...
      <div className="mbody">
        ...
        <div className="access-preview"><div className="section-label" style={{marginBottom:8}}>Access Preview</div><div className="text-sm" style={{lineHeight:1.8}}>
          ...
          f.role==="dept_head"?<><span className="text-green">✓</span> {(f.visible_departments||[]).length>0?`Can view: ${f.visible_departments.map(d=>DEPT[d]?.label).filter(Boolean).join(', ')}`:`${f.department?DEPT[f.department]?.label:"Department"} jobs only`}<br/><span className="text-green">✓</span> Full job actions (incl. delete) & reports for visible departments<br/><span className="text-red">✕</span> Cannot access Settings</>:
          <><span className="text-green">✓</span> {(f.visible_departments||[]).length>0?`Can view: ${f.visible_departments.map(d=>DEPT[d]?.label).filter(Boolean).join(', ')}`:`${f.department?DEPT[f.department]?.label:"Department"} jobs only`}<br/><span className="text-green">✓</span> Full job actions (create/edit/status/cancel/archive)<br/><span className="text-red">✕</span> Cannot delete jobs, or access Reports/Finance/Settings</>}
        </div></div>
      </div>
...
export default function Settings(){
  ...
  const ac=users.filter(u=>u.active).length;const bc=users.filter(u=>u.role==="bod"&&u.active).length;const dc=users.filter(u=>u.role==="dept_head"&&u.active).length;const sc=users.filter(u=>u.role==="staff"&&u.active).length;const inc=users.filter(u=>u.role==="intern"&&u.active).length;
  ...
  const caps=[["View all departments",1,0,0,0],["View own department(s)",1,1,1,1],["Create job",1,1,1,1],["Edit job",1,1,1,1],["Change status",1,1,1,1],["Archive/Cancel job",1,1,1,1],["Delete job/customer/vendor",1,1,0,0],["Manage customers/vendors",1,1,0,0],["Reports & Finance",1,1,0,0],["Export",1,1,0,0],["User Management",1,0,0,0],["Settings",1,0,0,0]];
