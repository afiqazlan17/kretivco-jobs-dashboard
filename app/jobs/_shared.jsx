...
// ─── Cancel Modal ────────────────────────────────────────────
// Closing a ticket that isn't going to reach Completed naturally — the
// everyday case being a quotation the customer never confirmed. Reuses
// the same reason list as before (still called CANCEL_REASONS
// internally); what changed is the framing and that the caller now
// tags which stage it was closed from (see closed_from_status).
export function CloseTicketModal({ job, onConfirm, onClose }) {
  ...
  return (
    <Modal width={440} onClose={guardedClose}>
      <div className="modal-header"><span className="modal-title">Close Ticket</span><button className="modal-close" onClick={guardedClose}>×</button></div>
      <div className="modal-body">
        ...
        {reason === 'other' && (
          <div>
            ...
            <input className="field-input" value={customText} onChange={e => setCustomText(e.target.value)} placeholder="Reason for closing..." />
          </div>
        )}
      </div>
      <div className="modal-footer">
        <button className="btn-secondary" onClick={guardedClose}>Cancel</button>
        <button className="btn-primary" style={{ background: '#EF4444' }} onClick={() => onConfirm(reason, customText)}>Yes, Close Ticket</button>
      </div>
    </Modal>
  );
}
...
export function ActionMenu({ job, onTakeIn, onChangeResponsible, onCloseJob, onHold, onResume, onCloseTicket, onArchive }) {
  const [open, setOpen] = useState(false);
      ...
      {open && (
        <>
          ...
          <div style={{ position: 'absolute', top: '110%', right: 0, zIndex: 31, background: '#fff', borderRadius: 10, boxShadow: '0 8px 24px rgba(0,0,0,.18)', minWidth: 220, overflow: 'hidden', padding: '4px 0' }}>
            ...
            {canCloseTicket && <div style={{ borderTop: '1px solid #F0ECF4', margin: '4px 0' }} />}
            {canCloseTicket && item('Close Ticket', '✕', onCloseTicket, '#EF4444')}
