...
export default function Dashboard() {
  ...
  const inProgressPipeline = formatRM(jobs.filter(j => !j.archived && ['active','in_progress'].includes(j.status)).reduce((s,j) => s + (j.estimation_value||0), 0))
  ...
  return <>
    ...
    <div style={{ padding:24, maxWidth:1200, margin:'0 auto' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:16, marginBottom:24 }}>
        ...
        <StatCard icon="🔄" label="In Progress" value={stats.in_progress_jobs||0} sub="Dalam proses" color="#F59E0B" />
      ...
      <div style={{ display:'flex', gap:16, marginBottom:24, flexWrap:'wrap' }}>
        {[{l:'Pipeline Value',v:formatRM(stats.pipeline_value),s:'Active + In Progress',c:'#E91E63'},{l:'Potential Value',v:formatRM(stats.potential_value),s:'Belum confirm',c:'#6366F1'},{l:'Actual Revenue',v:formatRM(stats.actual_revenue),s:`${stats.completed_jobs||0} job selesai`,c:'#10B981'}].map((c,i)=>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:24 }}>
        ...
        <Card style={{ padding:'20px 24px' }}>
          ...
          <p style={{fontSize:12,color:'#6B6080',marginBottom:20}}>Potential → Active → In Progress → Selesai</p>
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            {[{l:'Potential',n:stats.potential_jobs||0,v:formatRM(stats.potential_value),c:'#6366F1'},{l:'Active',n:stats.active_jobs||0,v:formatRM(stats.pipeline_value),c:'#10B981'},{l:'In Progress',n:stats.in_progress_jobs||0,v:inProgressPipeline,c:'#F59E0B'},{l:'Selesai',n:stats.completed_jobs||0,v:formatRM(stats.actual_revenue),c:'#6B7280',last:true}].map((s,i) =>
