...
async function renderDoc(type, meta, customer) {
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(customerCompany ? `${customerName} (${customerCompany})` : (customerName || '—'), LEFT, 173.4)
  if (addressLine1) doc.text(addressLine1, LEFT, 186.4)
  if (addressLine2) doc.text(addressLine2, LEFT, 199.4)

  labelValueLeft(doc, 'Title: ', jobTitle || '—', LEFT, 221.4)
  let y = 234.1
