...
async function renderDoc(type, meta, customer) {
  ...
  const stdCols = { 0: { cellWidth: 35, halign: 'center' }, 1: { cellWidth: 228 }, 2: { cellWidth: 60, halign: 'center' }, 3: { cellWidth: 70, halign: 'right' }, 4: { cellWidth: 75, halign: 'right' } }
  ...
  let lineY = ny + 15
  let firstLine = true
  const noteLines = (meta.notesOverride && meta.notesOverride.length) ? meta.notesOverride : notesFor(type, bank)
  ...
  // ── Output ── default is a straight download; 'print' opens the PDF in a
  // new tab with the browser's print dialog triggered automatically instead
  // (autoPrint() flags the file so Chrome/Edge's built-in viewer opens it);
  // 'share' skips both, since the caller is about to hand the blob to
  // navigator.share() (WhatsApp) or its own download fallback instead.
  const filename = `${cfg.prefix}_${meta.filenamePart || 'document'}_${new Date().toISOString().slice(0, 10)}.pdf`
  if (meta.action === 'print') {
    doc.autoPrint()
    window.open(doc.output('bloburl'), '_blank')
  } else if (meta.action !== 'share') {
    doc.save(filename)
  }
