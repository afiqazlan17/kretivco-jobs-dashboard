...
export function DataProvider({ children }) {
  ...
  const addUser = (u) => {
    setUsers(p => {
      const nextN = p.reduce((max, x) => {
        const n = parseInt(x.staff_id?.slice(4), 10)
        return Number.isFinite(n) && n > max ? n : max
      }, 0) + 1
      const staff_id = u.staff_id || `EMP-${String(nextN).padStart(3, '0')}`
      return [...p, { ...u, staff_id }]
    })
    // Note: live mode user creation requires Supabase Auth admin — handled separately.
    // The DB trigger assigns staff_id server-side once that row is actually inserted.
  }
  ...
  return (
    <DataContext.Provider value={{
      ...
      addJob, updateJob, addCustomer, updateCustomer, addVendor, updateVendor, addUser, updateUser,
