...
import { MOCK_JOBS, MOCK_CUSTOMERS, MOCK_VENDORS, MOCK_USERS, MOCK_ACTIVITY, MOCK_PROFILE, enrichJobs, reconcileMockUsers } from './mock-data'
import { daysUntil, cogsAccount, opexAccount, EXPENSE_CATEGORIES, loanAccount, BANK } from './constants'
import { planReversal, decideDocPosting, computeInvoiceAmount, computeReceiptAmount, buildInvoiceEntry, buildReceiptEntry } from './ledger'

// Live mode writes are fire-and-forget against Supabase — the local state
// updates optimistically, and until now a failed write (RLS denial, network
// blip, etc.) only ever reached console.error. That meant a change could
// look saved, then silently vanish on the next reload, with no visible sign
// anything went wrong. SYNC_ERROR_EVENT lets any write function broadcast a
// failure; AppShell listens globally and shows it as a banner so a failed
// save is never invisible again.
export const SYNC_ERROR_EVENT = 'kretivco:sync-error'
function reportSyncError(action, error) {
  console.error(`${action} error:`, error)
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent(SYNC_ERROR_EVENT, { detail: { action, message: error?.message || String(error) } }))
  }
}
  ...
  const [profile, setProfile] = useState(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (email) {
        ...
        const users = reconcileMockUsers(stored?.users)
...
export function DataProvider({ children }) {
  ...
  const fetchAll = useCallback(async () => {
    if (isMockMode) {
      ...
      setVendors(stored?.vendors ?? MOCK_VENDORS)
      setUsers(reconcileMockUsers(stored?.users))
      setActivityLogs(stored?.activityLogs ?? {})
      setLedgerEntries(stored?.ledgerEntries ?? [])
      setItemLibrary(stored?.itemLibrary ?? [])
      setDataLoading(false)
      return
    }
    const [jR, cR, vR, uR, aR, lR, iR] = await Promise.all([
      ...
      supabase.from('item_library').select('*').order('usage_count', { ascending: false }),
    ])
    if (jR.error) console.error('jobs fetch error:', jR.error)
    if (cR.error) console.error('customers fetch error:', cR.error)
    if (vR.error) console.error('vendors fetch error (table may not exist yet):', vR.error)
    ...
    if (iR.error) console.error('item_library fetch error (table may not exist yet):', iR.error)
    setItemLibrary(iR.data || [])
    setDataLoading(false)
  }, [])
  ...
  useEffect(() => { if (isMockMode && !dataLoading) saveStore({ jobs, customers, vendors, users, activityLogs, ledgerEntries, itemLibrary }) }, [jobs, customers, vendors, users, activityLogs, ledgerEntries, itemLibrary, dataLoading])
  ...
  const addLog = (jobId, entry, jobUuid) => {
    ...
    if (!isMockMode && (entry.action === 'note' || entry.action === 'document_generated')) {
      ...
      }).then(({ error }) => { if (error) reportSyncError('addLog', error) })
    }
  }
  ...
  const addJob = (job, userName) => {
    ...
    if (!isMockMode) {
      ...
      supabase.from('jobs').insert({ ...dbJob, id: dbJob.id, created_by: null }).then(({ error }) => { if (error) reportSyncError('addJob', error) })
    }
  }
  ...
  const updateJob = (id, updates, userName, logEntry) => {
    ...
    if (!isMockMode) {
      ...
      supabase.from('jobs').update({ ...clean, updated_at: new Date().toISOString() }).eq('id', id).then(({ error }) => { if (error) reportSyncError('updateJob', error) })
    }
  }

  // Permanent deletion — unlike Archive/Cancel (a status flag, reversible),
  // this actually removes the row. A reason is mandatory and gets written
  // to activity_log *before* the delete, independent of addLog()'s normal
  // note/document_generated-only persistence gate, so the record survives
  // even in live mode. The log row's job_id goes null once its job is
  // gone (FK is ON DELETE SET NULL, not CASCADE) but job_code is a
  // snapshot taken at write time, so the entry stays identifiable.
  const deleteJob = async (id, reason, userName) => {
    const job = enrichedJobs.find(j => j.id === id) || jobs.find(j => j.id === id)
    if (!job || !reason?.trim()) return false
    const detail = `Job ${job.job_id}${job.customer_name ? ` (${job.customer_name})` : ''} deleted — Reason: ${reason.trim()}`
    if (!isMockMode) {
      const { error: logError } = await supabase.from('activity_log').insert({
        job_id: id, job_code: job.job_id, action: 'deleted', user_name: userName || 'System', detail,
      })
      if (logError) { reportSyncError('deleteJob (log)', logError); return false }
      const { error } = await supabase.from('jobs').delete().eq('id', id)
      if (error) { reportSyncError('deleteJob', error); return false }
    } else {
      setActivityLogs(prev => ({ ...prev, [job.job_id]: [...(prev[job.job_id] || []), { action: 'deleted', user: userName || 'System', detail, time: new Date().toISOString() }] }))
    }
  ...
  const addCustomer = (c, userName) => {
    setCustomers(p => [...p, c])
    if (!isMockMode) {
      supabase.from('customers').insert({ ...c }).then(({ error }) => { if (error) reportSyncError('addCustomer', error) })
    }
  }

  const updateCustomer = (id, updates) => {
    setCustomers(p => p.map(c => c.id === id ? { ...c, ...updates } : c))
    if (!isMockMode) supabase.from('customers').update(updates).eq('id', id).then(({ error }) => { if (error) reportSyncError('updateCustomer', error) })
  }

  const addVendor = (v) => {
    setVendors(p => [...p, v])
    if (!isMockMode) {
      supabase.from('vendors').insert({ ...v }).then(({ error }) => { if (error) reportSyncError('addVendor', error) })
    }
  }

  const updateVendor = (id, updates) => {
    setVendors(p => p.map(v => v.id === id ? { ...v, ...updates } : v))
    if (!isMockMode) supabase.from('vendors').update(updates).eq('id', id).then(({ error }) => { if (error) reportSyncError('updateVendor', error) })
  }

  // ── Item Library ──
  // Per-department reusable catalog of item name + description (no price —
  // pricing always varies per job, and typing the same line-item text every
  // quotation was the actual pain point). Search-first: staff look for the
  // item, pick it if it exists, or type+save a new one which joins the
  // library for next time. Matching is case-insensitive per department.
  const saveLibraryItem = (department, itemName, description, userId) => {
    const name = itemName?.trim()
    if (!department || !name) return
    const existing = itemLibrary.find(i => i.department === department && i.item_name.trim().toLowerCase() === name.toLowerCase())
    if (existing) {
      const usage_count = (existing.usage_count || 1) + 1
      setItemLibrary(p => p.map(i => i.id === existing.id ? { ...i, usage_count } : i))
      if (!isMockMode) supabase.from('item_library').update({ usage_count }).eq('id', existing.id).then(({ error }) => { if (error) reportSyncError('saveLibraryItem', error) })
      return existing
    }
    const row = { id: crypto.randomUUID(), department, item_name: name, description: description?.trim() || null, usage_count: 1, created_at: new Date().toISOString(), created_by: userId || null }
    setItemLibrary(p => [...p, row])
    if (!isMockMode) supabase.from('item_library').insert(row).then(({ error }) => { if (error) reportSyncError('saveLibraryItem', error) })
    return row
  }
  ...
  const updateUser = (id, updates) => {
    ...
    if (!isMockMode) {
      ...
      if (Object.keys(dbUpdates).length) supabase.from('users').update(dbUpdates).eq('id', id).then(({ error }) => { if (error) reportSyncError('updateUser', error) })
    }
  }
  ...
  const genVendorId = () => `KVE-${String(vendors.length + 1).padStart(3, '0')}`
  ...
  const addLedgerEntry = (entry) => {
    ...
    if (!isMockMode) supabase.from('ledger_entries').insert(row).then(({ error }) => { if (error) reportSyncError('addLedgerEntry', error) })
    return row
  }
  ...
  const reverseEntries = (predicate, userName) => {
    const { targetIds, reversals } = planReversal(ledgerEntries, predicate, userName)
    reversals.forEach(r => addLedgerEntry(r))
    if (targetIds.length) {
      setLedgerEntries(p => p.map(e => targetIds.includes(e.id) ? { ...e, reversed: true } : e))
      if (!isMockMode) targetIds.forEach(id => supabase.from('ledger_entries').update({ reversed: true }).eq('id', id).then(({ error }) => { if (error) reportSyncError('reverseEntries', error) }))
    }
  }
  ...
  //
  // Printing, downloading, or WhatsApp-ing a copy of the SAME invoice (same
  // amount already posted) is just producing another copy for delivery —
  // nothing about the transaction changed, so there's nothing to re-post.
  // Only reverse+repost when the amount actually differs from what's
  // currently on the ledger, or nothing's been posted for this job yet.
  const postInvoiceEntry = (job, docNumber, userName, amountOverride) => {
    const amount = computeInvoiceAmount(job, amountOverride)
    const decision = decideDocPosting(ledgerEntries, job.job_id, 'invoice', amount)
    if (decision === 'skip') return
    if (decision === 'unchanged') return ledgerEntries.find(e => e.job_id === job.job_id && e.type === 'invoice' && !e.reversed)
    reverseEntries(e => e.job_id === job.job_id && e.type === 'invoice', userName)
    return addLedgerEntry(buildInvoiceEntry(job, docNumber, amount, userName))
  }

  // Receipt issued: payment received, clears what was owed. Same supersede
  // rule as invoices — regenerating replaces the prior receipt, not adds to
  // it, and reprinting/resharing an unchanged amount doesn't touch the
  // ledger at all.
  const postReceiptEntry = (job, docNumber, userName, amountOverride) => {
    const amount = computeReceiptAmount(job, amountOverride)
    const decision = decideDocPosting(ledgerEntries, job.job_id, 'receipt', amount)
    if (decision === 'skip') return
    if (decision === 'unchanged') return ledgerEntries.find(e => e.job_id === job.job_id && e.type === 'receipt' && !e.reversed)
    const bank = job.bank || 'mbb'
    reverseEntries(e => e.job_id === job.job_id && e.type === 'receipt', userName)
    return addLedgerEntry(buildReceiptEntry(job, docNumber, amount, userName, bank))
  }

  // Plain-language expense entry — department set = cost of service for
  // that department, blank = company-wide operating expense.
  const postExpenseEntry = ({ category, department, jobId, amount, bank, date, notes, receiptPath, receiptName }, userName) => {
    const amt = Number(amount) || 0
    if (!amt || !bank) return
    const debitAccount = department ? cogsAccount(department, category) : opexAccount(category)
    ...
    return addLedgerEntry({
      ...
      receipt_path: receiptPath || null, receipt_name: receiptName || null,
    })
  }
  ...
  // them back) — tracked as a liability per-director (loan_<slug>), same
  // idea as the AR/bank accounts, just on the other side of the balance
  // sheet. 'in' = director → company (raises the liability); 'repayment' =
  // company → director (pays it down).
  const postDirectorLoan = ({ direction, directorName, amount, bank, date, notes }, userName) => {
    const amt = Number(amount) || 0
    if (!amt || !bank || !directorName?.trim()) return
    const account = loanAccount(directorName)
    const isIn = direction === 'in'
    return addLedgerEntry({
      date: date || new Date().toISOString(), type: isIn ? 'director_loan_in' : 'director_loan_repayment',
      description: `${isIn ? 'Loan from' : 'Loan repayment to'} ${directorName.trim()}${notes?.trim() ? ' — ' + notes.trim() : ''}`,
      department: null, job_id: null, doc_number: null,
      debit_account: isIn ? `bank_${bank}` : account,
      credit_account: isIn ? account : `bank_${bank}`,
      amount: amt, bank, created_by: userName || 'System',
    })
  }
  ...
  // Revenue/Expense/Equity (unlike Director Loan, which does hit a
  // liability account on one side).
  const postBankTransfer = ({ fromBank, toBank, amount, date, notes }, userName) => {
    const amt = Number(amount) || 0
    if (!amt || !fromBank || !toBank || fromBank === toBank) return
    return addLedgerEntry({
      date: date || new Date().toISOString(), type: 'bank_transfer',
      description: `Transfer ${BANK[fromBank]?.label || fromBank} → ${BANK[toBank]?.label || toBank}${notes?.trim() ? ' — ' + notes.trim() : ''}`,
      department: null, job_id: null, doc_number: null,
      debit_account: `bank_${toBank}`, credit_account: `bank_${fromBank}`,
      amount: amt, bank: null, created_by: userName || 'System',
    })
  }

  return (
    <DataContext.Provider value={{
      jobs: enrichedJobs, rawJobs: jobs, customers, vendors, users, dataLoading,
      addJob, updateJob, deleteJob, addCustomer, updateCustomer, addVendor, updateVendor, addUser, updateUser,
      getActivity, genJobId, genCustId, genVendorId, genProjectId, resetAll, resetJobs, addLog, refetch: fetchAll,
      ledgerEntries, postInvoiceEntry, postReceiptEntry, postExpenseEntry, postOpeningBalanceAdjustment, postDirectorLoan, postBankTransfer, reverseJobLedgerEntries,
      itemLibrary, saveLibraryItem,
    }}>
      {children}
...
export function useDashboard() {
  ...
  const stats = {
    ...
    pipeline_value: jobs.filter(j => !j.archived && j.status === 'in_progress').reduce((s,j) => s + (j.estimation_value||0), 0),
