...
export function DataProvider({ children }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    ;(aR.data || []).forEach(l => {
      ...
      logs[key].push({ action: l.action, user: l.user_name || 'System', detail: l.detail || l.note, field: l.field_changed, old: l.old_value, val: l.new_value, time: l.created_at })
    })
  ...
  useEffect(() => { if (isMockMode) saveStore({ jobs, customers, users, activityLogs, ledgerEntries }) }, [jobs, customers, users, activityLogs, ledgerEntries])

  const enrichedJobs = enrichJobs(jobs, customers)

  // ── Activity Logging ──
  // Local-only optimistic log — DB triggers handle Supabase-side logging
  ...
  // Invoice issued: revenue recognized, amount owed by the customer.
  const postInvoiceEntry = (job, docNumber, userName) => {
    const amount = (job.line_items || []).reduce((s, li) => s + ((Number(li.qty) || 0) * (Number(li.price) || 0)), 0) || job.estimation_value || 0
    if (!amount) return
  ...
  // Receipt issued: payment received, clears what was owed.
  const postReceiptEntry = (job, docNumber, userName) => {
    const amount = job.final_value || job.estimation_value || 0
    if (!amount) return
  ...
  // Void every not-yet-reversed entry tied to a job (e.g. on cancellation) by
  // posting the mirrored (debit/credit swapped) counter-entry — never deletes
  // the original, so the audit trail stays intact.
  const reverseJobLedgerEntries = (jobId, userName) => {
    ledgerEntries.filter(e => e.job_id === jobId && !e.reversed).forEach(orig => {
      addLedgerEntry({
  ...
  return (
    <DataContext.Provider value={{
      ...
      getActivity, genJobId, genCustId, genProjectId, resetAll, addLog, refetch: fetchAll,
