...
export function DataProvider({ children }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [jR, cR, vR, uR, aR, lR] = await Promise.all([
  ...
  useEffect(() => { if (isMockMode && !dataLoading) saveStore({ jobs, customers, vendors, users, activityLogs, ledgerEntries }) }, [jobs, customers, vendors, users, activityLogs, ledgerEntries, dataLoading])
  ...
  const reverseEntries = (predicate, userName) => {
    ledgerEntries.filter(e => predicate(e) && !e.reversed).forEach(orig => {
      ...
      if (!isMockMode) supabase.from('ledger_entries').update({ reversed: true }).eq('id', orig.id).then(({ error }) => { if (error) reportSyncError('reverseEntries', error) })
    })
  }
  ...
  const postInvoiceEntry = (job, docNumber, userName, amountOverride) => {
    ...
    if (!amount) return
    const existing = ledgerEntries.find(e => e.job_id === job.job_id && e.type === 'invoice' && !e.reversed)
  ...
  const postReceiptEntry = (job, docNumber, userName, amountOverride) => {
    ...
    const existing = ledgerEntries.find(e => e.job_id === job.job_id && e.type === 'receipt' && !e.reversed)
    if (existing && Number(existing.amount) === amount) return existing
