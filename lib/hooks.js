...
export function DataProvider({ children }) {
  ...
  const addVendor = (v) => {
    setVendors(p => [...p, v])
    if (!isMockMode) {
      supabase.from('vendors').insert({ ...v }).then(({ error }) => { if (error) console.error('addVendor error:', error) })
    }
  }

  const updateVendor = (id, updates) => {
    setVendors(p => p.map(v => v.id === id ? { ...v, ...updates } : v))
    if (!isMockMode) supabase.from('vendors').update(updates).eq('id', id).then()
  }
