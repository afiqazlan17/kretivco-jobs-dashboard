...
export async function signIn(email, password) {
  if (isMockMode) {
    ...
    // Some browsers (Safari Private Browsing, in-app webviews opened from
    // chat apps) throw on localStorage writes instead of silently no-op-ing
    // — without this try/catch, that would make every single login attempt
    // fail with the generic "Could not log in" error, not just this one.
    // The session just won't survive a page reload in that environment.
    try { if (typeof window !== 'undefined') localStorage.setItem('mock_user_email', user.email) } catch {}
    return { user }
  }
...
export async function signOut() {
  if (isMockMode) { try { if (typeof window !== 'undefined') localStorage.removeItem('mock_user_email') } catch {}; return }
