...
// Assalamualaikum only opens the Morning greeting — Malay convention is to
// lead with it once for the day, not repeat it every time of day.
const GREETING_WINDOWS = [
  { start: 1380, end: 1439, period: 'Good Night',     small: () => `Lewat malam ni, take care tau.` },
  { start: 0,    end: 299,  period: 'Good Night',     small: () => `Lewat malam ni, take care tau.` },
  { start: 300,  end: 479,  period: 'Good Morning',   small: () => `Awal lagi ni, jom mulakan hari dengan secawan kopi.` },
  { start: 480,  end: 509,  period: 'Good Morning',   small: () => `Kerja start pukul 8:30, dah bersedia?` },
...
export function greetingFor(name) {
  ...
  const n = name || 'there'
  const big = win.period === 'Good Morning' ? `Assalamualaikum & ${win.period}, ${n}.` : `${win.period}, ${n}.`
