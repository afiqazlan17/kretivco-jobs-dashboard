...
// ITIL-style lifecycle from "new" onward: a job stays "potential"
...
// (no PIC). Staff claim it ("Take In Job"), which sets PIC and moves it to
...
export function ledgerAccountLabel(key) {
  ...
  if (key?.startsWith('cogs_')) return `Cost of Service — ${DEPT[key.slice(5)]?.label || key.slice(5)}`
