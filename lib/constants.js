...
export function greetingFor(name) {
  const hour = Number(new Date().toLocaleString('en-US', { hour: 'numeric', hour12: false, timeZone: 'Asia/Kuala_Lumpur' }))
  const period = hour < 5 ? 'Good Night' : hour < 12 ? 'Good Morning' : hour < 18 ? 'Good Afternoon' : hour < 23 ? 'Good Evening' : 'Good Night'
  return name ? `Assalamualaikum & ${period}, ${name}` : `Assalamualaikum & ${period}`
}
