...
// items by hand — the chosen tier's price and item list get baked into the
// job's line_items at creation. Plain custom jobs (e.g. "print a banner")
// just skip this and go through the normal manual flow.
...
// Flatten a department's package catalog into single-dropdown options —
// value encodes "pkgKey:pcs" so a selection round-trips via findPackageTier.
export function packageTierOptions(dept) {
  const pkgs = PACKAGE_CATALOG[dept] || []
  return pkgs.flatMap(pkg => pkg.tiers.map(tier => ({
...
export function findPackageTier(dept, value) {
  if (!value) return null
  return packageTierOptions(dept).find(o => o.value === value) || null
}
