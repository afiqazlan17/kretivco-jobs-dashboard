...
export const BANK = {
  ...
  affin: { label: 'AFFIN',   code: 'AFFIN', color: '#E53935', acct: 'XXXX-XXXX-XXXX', name: 'KRETIVCO MEDIAWORKS' },
}
...
export const ROLE = {
  ...
  intern:    { label: 'Intern',    color: '#10B981', desc: 'Own department — same access as Staff' },
}
...
// Directors are a dynamic list (any BOD user), not a fixed enum like banks,
// so their loan account is keyed off a slug of their name rather than a
// short static code.
export const slugify = (name) => (name || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '')
export const loanAccount = (directorName) => `loan_${slugify(directorName)}`
...
export function ledgerAccountLabel(key) {
  ...
  if (key?.startsWith('loan_')) return `Director Loan — ${key.slice(5).split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}`
  return key || '—'
}
...
export function ledgerAccountMeta(key) {
  ...
  if (key?.startsWith('loan_')) return { code: `LI-LOAN-${key.slice(5).toUpperCase()}`, label: ledgerAccountLabel(key), type: 'Liability' }
